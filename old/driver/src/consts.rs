/// Directory for all source language examples
pub const EXAMPLES_DIR: &str = "examples";

/// Directory for out files
pub const OUT_DIR: &str = "target";
pub const ASSEMBLY_DIR: &str = "asm";
pub const OBJ_DIR: &str = "obj";
pub const EXE_DIR: &str = "bin";

pub const L_IF_DIR: &str = "l_if";
pub const L_VAR_DIR: &str = "l_var";
pub const L_VAR_REG_DIR: &str = "l_var_reg";
pub const L_INT_DIR: &str = "l_int";

/// c files
pub const C_DIR: &str = "include";
pub const LIB_C: &str = "runtime.o";
