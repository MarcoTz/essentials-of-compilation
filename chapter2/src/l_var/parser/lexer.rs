...
pub fn parse_var(input: &mut String) -> Result<Var, Error> {
    if input.is_empty() {
        return Err(Error::UnexpectedEOI);
    }
...
#[cfg(test)]
mod lexer_tests {
    use super::{consume_character, consume_sequence, consume_whitespace, parse_var};
    ...
    #[test]
    fn parse_identifier() {
        let result = parse_var(&mut "name rest".to_owned()).unwrap();
        let expected = "name";
        assert_eq!(result, expected)
    }
}
